function GolfScript(code,stack=[],blocks={},output='')
{
  ...
  coerce=(...a)=>(x,y)=>a[type(x=erce(x,y))](x,erce(y,x)),
  ...
  intersect=(a,b)=>join(dedup(S(a)).filter(e=>b.indexOf(e)+1)),
  diff=(a,b)=>dedup(a.concat(b)).filter(e=>!includes(a,e)||!includes(b,e)),
  find=(f,a)=>a.find(e=>push(e) || f() || +!!bool(pop())),
  group=(a,n)=>a2a({length:Math.floor((len(a)+1)/n)},(_,i)=>a.slice(i*n,i*n+n)),
  ...
  vars={
    ...
    '-':coerce((f,g)=>block(join(minus(a2a(S(f)),a2a(S(g))))),(s,t)=>join(minus(a2a(s),a2a(t))),minus,f2n((x,y)=>x-y)),
    ...
    '/':order([unfold],[(f,s)=>each(f,s2a(s)),(x,y)=>x.split(y)],[each,(s,a)=>split(s2a(s),a),split],
      [(c,n)=>unfold(c,block(S(n))),(s,n)=>a2a({length:Math.ceil(len(s)/n)},(_,i)=>s.substr(i*n,n)),group,f2n((x,y)=>x/y)]),
    ...
    '|':coerce((x,y)=>block(join(dedup(x+y))),(x,y)=>join(dedup(x+y)),(x,y)=>dedup(x.concat(y)),f2n((x,y)=>x|y)),
    '&':coerce((x,y)=>block(intersect(S(x),S(y))),intersect,(x,y)=>dedup(x).filter(e=>includes(y,e)),f2n((x,y)=>x&y)),
    '^':coerce((x,y)=>block(join(diff(S(x),S(y)))),com(join,diff),diff,f2n((x,y)=>x^y)),
    ...
    '.':g=>push(g,g),
    ';':_=>{},
    '<':order([less],[less,less],[(f,a)=>aless(s2a(f),a),(s,a)=>aless(s2a(s),a),aless],[(f,n)=>block(S(f).slice(0,n)),(s,n)=>s.slice(0,n),(a,n)=>a.slice(0,n),less]),
    '>':order([more],[more,more],[(f,a)=>aless(a,s2a(f)),(s,a)=>aless(a,s2a(s)),(x,y)=>aless(y,x)],
      [(f,n)=>block(S(f).slice(n)),(s,n)=>s.slice(n),(a,n)=>a.slice(n),more]),
    '\\':(g,h)=>push(h,g),
    '[':()=>{lengths.push(len(stack))},
    ']':()=>stack.splice(lengths.pop()),
    '?':order([],[(f,s)=>find(f,s2a(s)),(x,y)=>x.indexOf(y)],[find,(s,a)=>index(a,s),index],[9,(s,n)=>s.indexOf(a2s(n)),index,f2n((x,y)=>x**y)]),
