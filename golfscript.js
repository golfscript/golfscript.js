function GolfScript(code,stack=[],blocks={},output='')
{
  ...
  corev=(n,a,s=com(join,a),f=com(block,s,S))=>coerce(f,s,a,f2n(n)), // reverse & supply defaults
  ...
  vars={
    ...
    '-':corev((x,y)=>x-y,minus,com(join,minus,a2a)),
    ...
    '|':corev((x,y)=>x|y,com(dedup,concat)),
    '&':corev((x,y)=>x&y,(a,b)=>dedup(a).filter(e=>includes(b,e))),
    '^':corev((x,y)=>x^y,(a,b)=>dedup(concat(a,b)).filter(e=>!includes(a,e)||!includes(b,e))),
